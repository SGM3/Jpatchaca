...
public class JiraImpl implements Jira {
	...
	@Override
	public void newWorklog(final String key, final Calendar startDate,
			final int timeSpent) {

		final RemoteWorklog workLog = new RemoteWorklog();
		workLog.setStartDate(startDate);
		workLog.setTimeSpent(JiraUtil.humanFormat(timeSpent));

		jiraService.addWorklogAndAutoAdjustRemainingEstimate(key, workLog);
	}
