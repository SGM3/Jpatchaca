...
public class JiraSystemImpl implements JiraSystem {
	...
	void logWorkOnIssue(final Period period, final String issueKey) {
		...
		final int duration = worklogOverride.getMinutesDuration(period);
		if (duration <= 0)
			throw new JiraException("Period Invalid For Issue " + issueKey + " " + worklogOverride.getHumanFormatDuration(period));
